...
from chives.types.weight_proof import SubEpochData, WeightProof
from chives.util.byte_types import hexstr_to_bytes
from chives.util.chunks import chunks
from chives.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from chives.util.ints import uint32, uint64
from chives.util.keychain import Keychain, KeyringIsLocked
...
from chives.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from chives.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
...
class WalletNode:
    ...
    async def _process_new_subscriptions(self):
        while not self._shut_down:
            ...
            peer: Optional[WSChivesConnection] = None
    ...
    async def is_peer_synced(
        self, peer: WSChivesConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSChivesConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSChivesConnection]:
        if self.server is None:
            return None
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSChivesConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSChivesConnection):
        if self.wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSChivesConnection) -> int:
        assert self.wallet_state_manager is not None
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSChivesConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_puzzle_solution(self, peer: WSChivesConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSChivesConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
    ...
    async def fetch_children(
        self, peer: WSChivesConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
